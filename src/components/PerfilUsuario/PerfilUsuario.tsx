...
export const PerfilUsuario: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchPessoa() {
      if (pessoaId) {
        ...
        const contasRes = await import("../../services/api").then((m) =>
          m.api.get("/contas")
        );

        const conta = contasRes.data.find(
          (c: any) => c.userId && c.userId._id === userId
        );

        if (conta) {
          // 2. Buscar pessoas e encontrar a pessoa que tem a conta
          const pessoasRes = await import("../../services/api").then((m) =>
            m.api.get("/pessoas")
          );

          const pessoa = pessoasRes.data.find(
            (p: any) => p.contaId === conta._id
          );
  ...
  return (
    <div className={styles.container}>
      ...
      <PerfilHeaderSecao
        pessoa={pessoa}
        aoClicarEditarPerfil={rolarParaFormulario}
      />
